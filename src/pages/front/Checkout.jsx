  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (paymentMethod === 'credit') {
        ...
        if (!data.cardNumber || !data.cvc) { //防呆
          Swal.fire({
            title: '請輸入完整的信用卡資訊',
            icon: 'warning'
          });
      ...
      <div className="modal-dialog modal-dialog-centered text-center">
        <div className="modal-content">
          ...
          <div className="modal-body">
            <img src="https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=FakeQRCode" alt="QRCode" width={200} height={200} />
            <button
                type="button"
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <div className="form-check">
                  <input className="form-check-input" type="radio" id="credit"
                    value="credit"
                    {...register('payment')}
                    checked={paymentMethod === 'credit'}
                    onChange={() => setPaymentMethod('credit')}
                  />
                  <label className="form-check-label" htmlFor="credit">信用卡付款</label>
                </div>
                ...
                <div className="form-check">
                  <input className="form-check-input" type="radio" id="apple"
                    value="apple"
                    {...register('payment')}
                    checked={paymentMethod === 'apple'}
                    onChange={() => setPaymentMethod('apple')}
                  />
                  <label className="form-check-label" htmlFor="apple">Apple Pay</label>
                </div>
                <div className="form-check">
                  <input className="form-check-input" type="radio" id="line"
                    value="line"
                    {...register('payment')}
                    checked={paymentMethod === 'line'}
                    onChange={() => setPaymentMethod('line')}
                  />
                  <label className="form-check-label" htmlFor="line">Line Pay</label>
                </div>
              </div>

            </div>
            <div className='d-flex flex-row flex-md-row mt-4 justify-content-between align-items-md-center align-items-end w-100'>
              <Link className='btn btn-primary rounded-2 text-white mt-md-0 mt-3 fw-bold' to='/cart'>
            ...
            <div className='border p-4 mb-4'>
                  ...
                  <div className='d-flex mb-3' key={item.id}>
                    ...
                    <div className='w-100'>
                      ...
                      <div className='d-flex justify-content-between'>
                        ...
                        <p className='mb-0'>NT$ {(item.product.price * item.qty).toLocaleString()}</p>
                      </div>
                    </div>
                  </div>
                );
              ...
              <div className='d-flex justify-content-start mt-2 text-danger'>
                <p className='h6 fw-bold mt-2'>
                  折扣: {discountPercent > 0 ? `- ${discountPercent}%` : '0%'} (- NT${discountAmount.toLocaleString()})
                </p>
              </div>
