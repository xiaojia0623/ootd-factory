...
const CartPage = () => {
    ...
    return (
        <div className='container full-height bg-white my-5'>
            <div className='row justify-content-center'>
                ...
                {cartData?.carts?.length === 0 ? (
                    ...
                    <Link to='/products' className='btn btn-primary text-white w-50 mt-4 rounded-0 fs-5 py-3'>來去選擇喜歡的商品</Link>
                        ...
                        <>
                            <div className='col-md-12 col-lg-12 bg-white  full-height'>
                                <div className='cart-confirm d-flex justify-content-between '>
                                    <h2 className='mt-2'>您的購物車<span className='fs-6 ms-2 text-danger fw-bold'>請確認商品以及數量</span></h2>
                                    <Link to='/products' type='button' className='btn btnLink fw-bold p-2'>再去逛逛</Link>
                                </div>
                                <div className='row gap-md-3 d-flex justify-content-start '>
                                    { cartData?.carts?.map((item) => {
                                        return (
                                            <div className='col col-md-12 col-lg-5 d-flex mt-4 bg-light' key={item.id}>
                                                <img
                                                ...
                                                <div className='w-100 p-3 position-relative'>
                                                    <button type='button' className='btn position-absolute' style={{ top: '16px', right: '16px' }} onClick={() => removeCartItem(item.id)}>
                                                        <i className="bi bi-x-lg"></i>
                                                    </button>
                                                    <p className='mb-0 fw-bold fs-5'>{item.product.title}</p>
                                                    <p className='text-muted' style={{ fontSize: '12px', marginTop:'10px' }}>{item.product.content}</p>
                                ...
                                <div className="d-flex align-items-center gap-3 mt-3">
                                    ...
                                    disabled={isApplying}
                                    />
                                    <button className="btn btn-primary " onClick={applyCoupon} disabled={isApplying}>
                                ...
                                <div className='d-flex justify-content-start mt-4  position-relative top-0'>
                                    <p className='mb-0 h6 fw-bold'>運費: </p>
                                ...
                                <Link
                                    to='/checkout'
                                    className='comfirm-btn btn btn-primary text-white fw-bold fs-6 mt-4 rounded-0 py-3'
                                >
                                    確認產品正確
                                </Link>
                            </div>    
